...
exports.fetchTrendsIncremental = fetchTrendsIncremental;
 ...
 */
function convertTimeWindowToFreshness(timeWindow) {
    if (!timeWindow)
        return undefined;
 ...
 */
async function* searchWithMCP(query, maxResults = 10, timeWindow) {
    if (!mcpClient) {
    ...
    const searchTools = tools.tools.filter(t => t.name.toLowerCase().includes("search"));
    // Prefer web_search over local_search
    const webSearch = searchTools.find(t => t.name.toLowerCase().includes("web") ||
 ...
 */
function determineCacheTTL(query, timeWindow) {
 ...
 */
function generateQueryHash(query, timeWindow, domain) {
 ...
 */
async function fetchTrendsIncremental(query, timeWindow, onTrendFound, domain, persona) {
    ...
    if (convex) {
        try {
            ...
            const cachedResult = await convex.query(api_1.api.queries.getCachedResearch, {
                queryHash,
                query,
                timeWindow,
                domain: domain || query,
            ...
            if (cachedResult && cachedResult.trends && cachedResult.trends.length > 0) {
                ...
                const filteredTrends = cachedResult.trends.filter((trend) => {
                ...
                if (filteredTrends.length === 0 && cachedResult.trends.length > 0) {
                    ...
                    const sortedCachedTrends = cachedResult.trends.sort((a, b) => {
    ...
    const trends = [];
    const allTrends = []; // Store all trends (including outside timeWindow) for fallback
    ...
    for await (const result of searchWithMCP(query, 10, timeWindow)) {
        ...
        const normalizedTimestamp = await normalizeWebsiteTimestamp(result.timestamp, timeWindow, result.url, cleanSnippet, cleanTitle);
    ...
    if (trends.length === 0 && allTrends.length > 0) {
        ...
        const sortedTrends = allTrends.sort((a, b) => {
            ...
            const timestampB = b.sources?.[0]?.timestamp;
            if (!timestampA)
                return 1; // Put items without timestamp at end
            if (!timestampB)
                return -1;
            return new Date(timestampB).getTime() - new Date(timestampA).getTime();
        });
    ...
    if (convex && trendsToCache.length > 0) {
        try {
            ...
            const validTrendsToCache = trendsToCache.filter((trend) => {
                const sourceTimestamp = trend.sources?.[0]?.timestamp;
                if (!sourceTimestamp) {
            ...
            if (validTrendsToCache.length > 0) {
                ...
                const sortedForCache = validTrendsToCache.sort((a, b) => {
                    const timestampA = a.sources?.[0]?.timestamp;
                    const timestampB = b.sources?.[0]?.timestamp;
                    if (!timestampA)
                        return 1;
                ...
                await convex.mutation(api_1.api.mutations.saveResearchCache, {
                    queryHash,
                    query,
                    timeWindow,
                    domain: domain || query,
                ...
                const withinWindowCount = sortedForCache.filter((t) => isTimestampInTimeWindow(t.sources?.[0]?.timestamp, timeWindow)).length;
 ...
 */
async function extractDateFromURLAPI(url) {
    try {
 ...
 */
async function fetchTimestampFromMCP(url) {
    if (!mcpClient) {
        return undefined;
    }
    try {
        ...
        const metadataTools = tools.tools.filter(t => t.name.toLowerCase().includes('fetch') ||
 ...
 */
function isTimestampInTimeWindow(timestamp, timeWindow) {
    if (!timestamp)
        return false;
    try {
 ...
 */
async function normalizeWebsiteTimestamp(timestamp, timeWindow, url, snippet, title) {
 ...
 */
async function generateWhyItMatters(snippet, query, persona = "") {
    try {
 ...
 */
function calculateConfidence(result, timeWindow) {
    ...
    if (result.timestamp) {
        try {
            ...
            if (daysAgo < 1)
                confidence += 0.25; // Very recent (today)
            else if (daysAgo < 7)
                confidence += 0.2; // This week
            else if (daysAgo < 30)
                confidence += 0.15; // This month
            else if (daysAgo < 90)
                confidence += 0.1; // This quarter
            else if (daysAgo < 365)
                confidence += 0.05; // This year
    ...
    if (snippet.length > 150)
        confidence += 0.1; // Detailed content
    ...
    if (hasNumbers)
        confidence += 0.05; // Data-driven
    if (hasActionWords)
        confidence += 0.05; // Action-oriented
