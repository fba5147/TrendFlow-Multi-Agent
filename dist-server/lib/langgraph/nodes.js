...
exports.researchPlanningNode = researchPlanningNode;
exports.trendRetrievalNode = trendRetrievalNode;
exports.synthesisNode = synthesisNode;
exports.hitlCheckpointNode = hitlCheckpointNode;
exports.contentGenerationNode = contentGenerationNode;
 ...
 */
async function researchPlanningNode(state) {
    const persona = state.userPersona || "";
    const brandPrompt = (0, prompts_1.getGalliumAIBrandPrompt)(persona);
...
Response: { "timeWindow": "this week", "region": null, "domain": "creator economy trends", "tools": ["web_search"], "platforms": ["LinkedIn", "X"] }`;
    ...
    const messages = [
        ...
        new messages_1.HumanMessage(`Analyze this query and extract research parameters:\n\n"${queryText}"`),
    ];
    const response = await llm_1.llm.invoke(messages);
    ...
    try {
        ...
        if (!planData.platforms || !Array.isArray(planData.platforms) || planData.platforms.length === 0) {
            ...
            planData.platforms = planData.platforms.map((p) => {
                const normalized = (0, utils_1.normalizePlatformName)(p);
                return normalized;
        ...
        if (domainText.length > 50 || !domainText) {
            ...
            domainText = phrases.find(p => p.trim().length > 5 &&
                !p.match(/\b(this week|last week|recent|US|USA|give|me|show)\b/i))?.trim() || "trending topics";
    ...
    const researchPlan = {
        ...
        platforms: (planData.platforms && Array.isArray(planData.platforms) && planData.platforms.length > 0)
            ? planData.platforms.map((p) => (0, utils_1.normalizePlatformName)(p))
            : ["LinkedIn", "X"],
    };
    // Final validation
 ...
 */
async function trendRetrievalNode(state) {
    ...
    const onTrendFound = state.__onTrendFound;
    try {
        ...
        while (searchAttempts < maxSearchAttempts) {
            ...
            const highConfidenceTrends = uniqueTrendsSoFar.filter(t => (t.confidence || 0) >= CONFIDENCE_THRESHOLD);
            ...
            try {
                const persona = state.userPersona || "";
                const trends = await (0, client_1.fetchTrendsIncremental)(currentQuery, scope.timeWindow, onTrendFound, scope.domain, persona);
                if (trends && trends.length > 0) {
                    ...
                    const allHighConfTrends = uniqueAllTrends.filter(t => (t.confidence || 0) >= CONFIDENCE_THRESHOLD);
                    ...
                    const highConfFromThisSearch = uniqueTrendsFromThisSearch.filter(t => (t.confidence || 0) >= CONFIDENCE_THRESHOLD);
                    ...
                    const newTrendsCount = highConfFromThisSearch.filter(t => !t.sources.some(s => existingUrls.has(s.url.toLowerCase()))).length;
        ...
        // Log final status
        const finalHighConfidenceTrends = allTrends.filter(t => (t.confidence || 0) >= CONFIDENCE_THRESHOLD);
        if (searchAttempts >= maxSearchAttempts) {
        ...
        const highConfidenceTrends = uniqueTrends.filter(t => (t.confidence || 0) >= CONFIDENCE_THRESHOLD);
 ...
 */
function mergeSourcesFromSimilarTrends(trends) {
    const merged = [];
    ...
    for (let i = 0; i < trends.length; i++) {
        ...
        const bestTrend = similarTrends.reduce((best, current) => (current.confidence || 0) > (best.confidence || 0) ? current : best);
        merged.push({
            ...bestTrend,
...
async function synthesisNode(state) {
    ...
    const persona = state.userPersona || "";
    const systemPrompt = `${(0, prompts_1.getGalliumAIBrandPrompt)(persona)}
  
  ${(0, prompts_1.getTrendSynthesisPrompt)(persona)}`;
    ...
    const messages = [
        new messages_1.SystemMessage(systemPrompt),
        new messages_1.HumanMessage(`Analyze and rank these ${state.trends.length} trend${state.trends.length !== 1 ? 's' : ''} for marketing teams:\n\n${trendsText}\n\nFocus on:
...
IMPORTANT: Only return trends that match the titles provided above. Do not create new trends. Return exactly ${state.trends.length <= 10 ? state.trends.length : maxTrends} trend${(state.trends.length <= 10 ? state.trends.length : maxTrends) !== 1 ? 's' : ''} or fewer.`),
    ...
    try {
        ...
        try {
            ...
            rankedTrends = JSON.parse(jsonStr);
        ...
        const synthesizedTrends = rankedTrends
            .slice(0, maxTrendsToReturn) // Limit to original count or 10, whichever is smaller
            .map((ranked, index) => {
            // Try to match by exact title first
            let original = state.trends.find((t) => t.title.toLowerCase().trim() === ranked.title?.toLowerCase()?.trim());
            ...
            if (mergedSources.length < 3) {
                ...
                for (const similarTrend of similarTrends) {
                    if (mergedSources.length >= 4)
                        break; // Limit to 4 sources max
            ...
            .filter((t) => t !== null && t !== undefined && !!t?.title && !!t?.summary && (t.confidence || 0) >= 0.65); // Filter out trends below 0.65 confidence
 ...
 */
function findBestTrendMatch(targetTitle, trends) {
    if (!targetTitle || trends.length === 0)
        return undefined;
    ...
    for (const trend of trends) {
        ...
        if (minLength >= 10) {
            if (targetLower.includes(trendLower.substring(0, Math.min(20, trendLower.length))))
                score += 0.5;
            if (trendLower.includes(targetLower.substring(0, Math.min(20, targetLower.length))))
                score += 0.5;
        }
 ...
 */
function hitlCheckpointNode(state) {
 ...
 */
async function contentGenerationNode(state) {
    const onPlatformComplete = state.__onPlatformComplete;
    if (!state.approvedTrends || state.approvedTrends.length === 0) {
        throw new Error("No approved trends for content generation");
    }
    const platforms = (state.platforms || utils_1.MAIN_PLATFORMS).map(p => (0, utils_1.normalizePlatformName)(p));
    const contentIdeas = {};
    const persona = state.userPersona || "";
    const brandPrompt = (0, prompts_1.getGalliumAIBrandPrompt)(persona);
    ...
    for (const platform of platforms) {
        try {
            const platformPrompt = (0, prompts_1.getContentGenerationPrompt)(platform, persona);
...
[
  {
    ...
    "variants": [
      ...
      "Angle variant: Focus on the psychological barrier (fear of selling) rather than the data"
            ...
            const mappedIdeas = ideas
                ...
                if (!matchedTrend) {
                    // Find closest match
                    const match = trendTitles.find(title => title.toLowerCase().includes(trendRef.toLowerCase().substring(0, 20)) ||
                ...
                return {
                    ...
                    variants: Array.isArray(idea.variants) ? idea.variants.filter((v) => typeof v === 'string' && v.trim().length > 0).slice(0, 3) : [], // Limit to 3 variants, filter empty
                };
            })
    ...
    const normalizedExpected = new Set(platforms.map(p => (0, utils_1.normalizePlatformName)(p)));
    const normalizedCompleted = new Set(completedPlatforms.map(p => (0, utils_1.normalizePlatformName)(p)));
    ...
    const allPlatformsProcessed = platforms.length > 0 &&
        Array.from(normalizedExpected).every(p => {
            ...
            const found = Array.from(normalizedCompleted).some(completed => (0, utils_1.normalizePlatformName)(completed) === p);
            // Or check if it exists in contentIdeas (by original key)
            const hasInContentIdeas = Object.keys(contentIdeas).some(key => (0, utils_1.normalizePlatformName)(key) === p);
